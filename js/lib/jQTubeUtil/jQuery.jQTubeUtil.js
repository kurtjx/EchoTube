...
;jQTubeUtil = (function($){ /* singleton */
	 ...
	 * @param cb
	 */
	p.search = function(input, cb){
		if ( typeof(input) == "string" ) 
			input = { "q" : encodeURIComponent(input) };
		return _search($.extend({}, SearchDefaults, input), cb);
	};
	...
	var YouTubeVideo = function(entry){
		...
		try{ this.category = entry.media$group.media$category.$t; }catch(e){ unavail.push("category"); }
